...
with st.sidebar:
    ...
    st.slider(
        "\% of costs in paid in fiat", min_value=50, max_value=95, value=5, step=1, key="xr_locking_sensitivity",
        on_change=plot_ROI
    )
    st.slider(
        "XR locking sensitivity", min_value=1, max_value=95, value=60, step=1, key="pct_fiat_cost",
        on_change=plot_ROI
    )
